...
- `./scripts/tiffany-real-runtime-check [--run|--adapter-run] [--runtime claude|codex|gemini|all]` - opt-in check for installed/logged-in native CLIs. Default mode is quota-free and reports installed tools, model/auth preflight hints, native transcript stores, the orchestrator binary, and copyable next commands; `--run` sends tiny direct native prompts, and `--adapter-run` sends the same probes through `orchestrator events --worker ...` with temporary Tiffany state, then verifies the second same-role run reuses the same Tiffany worker thread/native session, `/thread export` writes a handoff file containing the worker result, and native history can be imported into/queryable from the session DB. Both run modes may consume model quota.
...
| `./scripts/tiffany-codex-tui-lib-test` | Run the full `codex-tui --lib` suite serially |
| `./scripts/tiffany-check-examples` | Run only checked-in example tests |
| `./scripts/tiffany-real-runtime-check [--run|--adapter-run] [--runtime claude|codex|gemini|all]` | Opt-in real native CLI/Tiffany adapter check; default is quota-free preflight, `--adapter-run` verifies same-role Tiffany/native session reuse, thread export, and native-history DB query |
...
- When Claude Code or Codex CLI reports `Session ID ... is already in use`, Tiffany waits and retries with the same native session to preserve continuity. If the saved session is missing or cannot be resumed, Tiffany clears that stale native session id and retries once with a fresh session. If it still fails, run `/thread <role>` to inspect the stored native session and `/thread clear <role>` to start the next run fresh.
...
# Existing-tag rewrites are blocked by default; prefer a new patch tag:
