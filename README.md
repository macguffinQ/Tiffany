...
- `/history` - inspect/search saved Tiffany native conversation turns and synced worker events such as tool output, stderr, and diff; use `/history role <role>` or `/history thread <id>` to focus one worker; `/history export [--out file.md]` writes a Markdown handoff
