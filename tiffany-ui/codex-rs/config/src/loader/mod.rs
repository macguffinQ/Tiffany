...
///
/// See https://github.com/macguffinQ/Tiffany/blob/main/tiffany-ui/docs/sandbox.md for details.
///
        ...
        && let Some(base_user_config) = base_user_layer.config.as_table()
    {
        ...
        if legacy_profile_is_selected || legacy_profile_table_exists {
            return Err(io::Error::new(
                ...
                format!(
                    "--profile `{active_user_profile}` cannot be used while {} contains legacy `profile = \"{active_user_profile}\"` or `[profiles.{active_user_profile}]` config; move those settings into {} and remove the legacy profile selector/table. See https://github.com/macguffinQ/Tiffany/blob/main/tiffany-ui/docs/config.md for more information.",
