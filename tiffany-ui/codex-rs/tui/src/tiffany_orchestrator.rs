            ...
            match item.get("type").and_then(|value| value.as_str()) {
                Some("tool_use") => {
                    if let Some(content) = native_cli_tool_call_text(item) {
                        let kind = native_cli_tool_call_kind(item);
                        events.push(native_cli_transcript_event(
                            command,
                            kind,
                            native_cli_tool_call_title(kind),
                            content,
                        ));
                    }
                }
                Some("tool_result") => {
                    if let Some(content) = native_cli_tool_result_text(item) {
                        ...
                        events.push(native_cli_transcript_event(
                            command,
                            kind,
                            native_cli_result_title(kind, "native tool result"),
                            content,
                        ));
                    }
                }
...
fn native_cli_result_title(kind: &str, default_title: &'static str) -> &'static str {
    match kind {
...
fn native_cli_tool_call_title(kind: &str) -> &'static str {
    match kind {
...
fn native_cli_tool_call_kind(item: &serde_json::Value) -> &'static str {
    let name = item
        .get("name")
        .and_then(json_string_value)
        .unwrap_or_default();
    let input = item.get("input");
    native_cli_named_tool_call_kind(&name, input)
}
...
fn native_cli_named_tool_call_kind(name: &str, input: Option<&serde_json::Value>) -> &'static str {
    let normalized = normalize_native_tool_name(name);
    if matches!(
        normalized.as_str(),
        "applypatch" | "apply_patch" | "patch" | "patchapply"
    ) || input.is_some_and(native_cli_tool_input_has_patch)
    {
        return "patch";
    }
    if matches!(
...
fn native_cli_tool_input_has_patch(value: &serde_json::Value) -> bool {
    match value {
        serde_json::Value::Object(map) => {
            map.get("patch").is_some()
                ...
                || map
                    .get("edits")
                    .is_some_and(native_cli_tool_input_has_patch)
        }
        serde_json::Value::Array(items) => items.iter().any(native_cli_tool_input_has_patch),
...
fn native_cli_error_kind(content: &str) -> &'static str {
    if native_cli_content_needs_approval(content) {
        "approval"
    } else {
        "stderr"
    }
...
fn codex_exec_end_kind(value: Option<&serde_json::Value>, result: &str) -> &'static str {
    ...
    if native_cli_content_needs_approval(result) {
        "approval"
    } else if failed {
        "stderr"
    } else {
...
fn native_cli_content_needs_approval(content: &str) -> bool {
    let lower = content.to_ascii_lowercase();
    if lower.trim().is_empty() || native_cli_content_is_service_error(&lower) {
        return false;
    }
...
fn native_cli_content_is_service_error(lower: &str) -> bool {
    [
        "api error",
        "api key",
        "authentication",
...
fn native_cli_tool_call_text(item: &serde_json::Value) -> Option<String> {
    ...
    let input = item
        .get("input")
        .map(native_cli_tool_input_summary)
        .unwrap_or_default();
...
fn native_cli_tool_input_summary(value: &serde_json::Value) -> String {
    match value {
        serde_json::Value::Object(map) => {
            let fields = [
                "file_path",
...
fn jobs_command_args(args: &str) -> Result<Vec<String>, String> {
    let parts = args.split_whitespace().collect::<Vec<_>>();
...
fn parse_role_option_summaries(text: &str) -> Vec<RoleOptionSummary> {
    text.lines()
