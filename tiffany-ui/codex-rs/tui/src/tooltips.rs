...
const APP_TOOLTIP: &str = "Use tiffany-loop from your terminal workflow.";
const FAST_TOOLTIP: &str =
    "*New* Use **/fast** to enable our fastest inference with increased plan usage.";
const OTHER_TOOLTIP: &str = "*New* Build faster with **tiffany-loop**.";
const OTHER_TOOLTIP_NON_MAC: &str = "*New* Build faster with tiffany-loop.";
const FREE_GO_TOOLTIP: &str = "*New* For a limited time, tiffany-loop is included in your plan for free – let’s build together.";
...
/// Pick a random tooltip to show to the user when starting tiffany-loop.
...
pub(crate) mod announcement {
    /// tiffany-loop disables remote announcement tips so startup never fetches upstream copy.
    ...
    impl TargetOs {
        const fn current() -> Self {
            if cfg!(target_os = "macos") {
                ...
                // tiffany-loop currently publishes CLI builds for macOS, Windows, and Linux.
                Self::Linux
            }
        }
    }
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for tiffany-loop TUI.
...
[[announcements]]
content = "Welcome to tiffany-loop! Check out the new onboarding flow."
