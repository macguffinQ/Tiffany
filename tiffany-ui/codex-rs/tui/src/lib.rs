...
use app_server_session::AppServerBootstrap;
...
pub use tiffany_orchestrator::TiffanyOrchestratorLaunch;
...
pub async fn run_main(
    ...
    let is_tiffany_orchestrator = cli.tiffany_orchestrator.is_some();
    ...
    if !is_tiffany_orchestrator && !app_server_target.uses_remote_workspace() {
        #[allow(clippy::print_stderr)]
...
async fn run_ratatui_app(
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        let skip_update_prompt =
            is_tiffany_orchestrator || cli.prompt.as_ref().is_some_and(|prompt| !prompt.is_empty());
    ...
    let login_status =
        if !is_tiffany_orchestrator && initial_config.model_provider.requires_openai_auth {
    ...
    let should_show_onboarding = !is_tiffany_orchestrator
    ...
    let startup_bootstrap = if is_tiffany_orchestrator {
        Some(tiffany_orchestrator_bootstrap(&config))
    } else {
        Some(app_server.bootstrap(&config).await?)
    };
...
fn tiffany_orchestrator_bootstrap(config: &Config) -> AppServerBootstrap {
    AppServerBootstrap {
        ...
        default_model: config
            .model
            .clone()
            .unwrap_or_else(|| "tiffany-orchestrator".to_string()),
        feedback_audience: bottom_pane::FeedbackAudience::External,
...
mod tests {
    ...
    fn run_large_stack_test(
        future: Pin<Box<dyn Future<Output = color_eyre::Result<()>>>>,
    ) -> color_eyre::Result<()> {
    ...
    fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        run_large_stack_test(Box::pin(async {
            let temp_dir = TempDir::new()?;
