...
#[tokio::test]
async fn tiffany_orchestrator_running_submission_queues_without_pending_steer() {
    let (mut chat, _rx, mut op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
...
async fn tiffany_orchestrator_intercept_clears_stale_pending_steers() {
    ...
    chat.input_queue
        .pending_steers
        .push_back(pending_steer("stale steer"));
    chat.input_queue.submit_pending_steers_after_interrupt = true;

    chat.on_tiffany_orchestrator_submission_intercepted();
