...
#[tokio::test]
async fn slash_roles_dispatches_to_tiffany_orchestrator() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_tiffany_orchestrator_shell(true);

    chat.dispatch_command_with_args(SlashCommand::Roles, "show critic".to_string(), Vec::new());

    match rx.try_recv() {
        Ok(AppEvent::TiffanyOrchestratorRolesCommand { args }) => {
            assert_eq!(args, "show critic");
        }
...
async fn tiffany_orchestrator_rejects_hidden_codex_command_when_dequeued() {
    ...
    queue_composer_text_with_tab(&mut chat, "/init");
...
async fn slash_role_bare_opens_registration_prompt() {
    ...
    assert!(popup.contains("Register role"));
    assert!(popup.contains("worker-codex"));
    assert!(popup.contains("API Model"));
    assert!(!popup.contains("Model ID"));
...
#[tokio::test]
async fn slash_role_single_arg_prefills_registration_prompt() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_tiffany_orchestrator_shell(true);

    chat.dispatch_command_with_args(SlashCommand::Role, "worker-cc".to_string(), Vec::new());
...
#[tokio::test]
async fn slash_role_provider_selection_filters_model_options() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_tiffany_orchestrator_shell(true);
    ...
    chat.handle_key_event(KeyEvent::new(KeyCode::Char('6'), KeyModifiers::NONE));

    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert!(popup.contains("deepseek"));
    assert!(popup.contains("deepseek-chat"));
    assert!(!popup.contains("MiniMax-M3"));
...
#[tokio::test]
async fn slash_role_custom_provider_allows_model_text_input() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_tiffany_orchestrator_shell(true);
    ...
    assert!(popup.contains("custom-modelx"));
    assert_matches!(rx.try_recv(), Err(TryRecvError::Empty));
}

#[tokio::test]
async fn slash_role_register_args_dispatch_directly() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_tiffany_orchestrator_shell(true);

    chat.dispatch_command_with_args(
        SlashCommand::Role,
        "register critic --model gpt4o --runtime codex".to_string(),
        Vec::new(),
    );

    match rx.try_recv() {
        Ok(AppEvent::TiffanyOrchestratorRolesCommand { args }) => {
            assert_eq!(args, "register critic --model gpt4o --runtime codex");
        }
        other => panic!("expected TiffanyOrchestratorRolesCommand, got {other:?}"),
    }
}
...
async fn slash_provider_dispatches_to_tiffany_orchestrator() {
    ...
    chat.dispatch_command_with_args(
        ...
        "key openai $OPENAI_API_KEY".to_string(),
        Vec::new(),
    );
...
#[tokio::test]
async fn slash_provider_bare_opens_setup_prompt() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_tiffany_orchestrator_shell(true);

    chat.handle_slash_command_dispatch(SlashCommand::Provider);

    assert_matches!(rx.try_recv(), Err(TryRecvError::Empty));
    let popup = render_bottom_popup(&chat, /*width*/ 90);
    assert!(popup.contains("Configure provider"));
    assert!(popup.contains("Preset"));
    assert!(popup.contains("Auth"));
...
#[tokio::test]
async fn slash_provider_setup_dropdown_selects_provider_defaults() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_tiffany_orchestrator_shell(true);
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert!(popup.contains("Options"));
    assert!(popup.contains("Space"));
    assert!(!popup.contains("Ctrl+O"));
    ...
    assert!(popup.contains("https://api.minimaxi.com/v1"));
    assert!(popup.contains("config provider setup minimax"));
    ...
    match rx.try_recv() {
        Ok(AppEvent::TiffanyOrchestratorProviderCommand { args }) => {
            assert_eq!(
                args,
                "setup minimax --type openai --env MINIMAX_API_KEY --endpoint https://api.minimaxi.com/v1"
            );
        }
        other => panic!("expected TiffanyOrchestratorProviderCommand, got {other:?}"),
    }
...
#[tokio::test]
async fn slash_provider_selection_fields_ignore_direct_typing() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_tiffany_orchestrator_shell(true);
...
#[tokio::test]
async fn slash_provider_edit_prefills_existing_provider_config() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
...
#[test]
fn provider_setup_draft_accepts_separate_fields() {
    let args = super::super::slash_dispatch::provider_setup_draft_args(
...
#[test]
fn provider_setup_draft_rejects_env_and_key_together() {
...
async fn pending_token_activity_refresh_keeps_composer_visible_in_short_viewport() {
    ...
    assert!(
        term.backend()
            ...
            .screen()
            .contents()
            .contains("Ask tiffany-loop to do anything")
    );
}
...
#[tokio::test]
async fn slash_logout_reports_provider_configuration_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    assert!(
        rendered.contains("tiffany-loop does not use account login"),
        "expected provider setup guidance, got: {rendered:?}"
    );
    assert_matches!(rx.try_recv(), Err(TryRecvError::Empty));
}
