...
impl ChatWidget {
    pub(super) fn handle_composer_input_result(
        ...
        match input_result {
            InputResult::Submitted {
                ...
                if self.should_queue_tiffany_orchestrator_submission() {
                    self.push_tiffany_orchestrator_queued_message(user_message);
                    return;
                }
    ...
    pub(crate) fn queue_tiffany_orchestrator_prompt(&mut self, prompt: String) {
        self.push_tiffany_orchestrator_queued_message(UserMessage::from(prompt));
    }

    fn should_queue_tiffany_orchestrator_submission(&self) -> bool {
        self.tiffany_orchestrator_shell && self.turn_lifecycle.agent_turn_running
    }

    fn push_tiffany_orchestrator_queued_message(&mut self, user_message: UserMessage) {
        self.input_queue
    ...
    pub(crate) fn maybe_send_merged_tiffany_queued_input(&mut self) -> bool {
        if self.input_queue.suppress_queue_autosend || self.is_user_turn_pending_or_running() {
            return false;
        }
        if !self.input_queue.rejected_steers_queue.is_empty() {
            return self.maybe_send_next_queued_input();
        }
        if self.input_queue.queued_user_messages.is_empty() {
            return false;
        }
        if self
            .input_queue
            .queued_user_messages
            .iter()
            .any(|message| message.action != QueuedInputAction::Plain)
        {
            return self.maybe_send_next_queued_input();
        }
