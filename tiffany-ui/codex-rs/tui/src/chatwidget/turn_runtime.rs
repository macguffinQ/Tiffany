...
impl ChatWidget {
    ...
    pub(crate) fn on_tiffany_orchestrator_submission_intercepted(&mut self) {
    ...
    pub(crate) fn on_tiffany_orchestrator_run_started(&mut self) {
        self.input_queue.user_turn_pending_start = false;
        self.turn_lifecycle.start(Instant::now());
        self.update_task_running_state();
        self.set_status_header(String::from("Orchestrating"));
        self.status_state.terminal_title_status_kind = TerminalTitleStatusKind::Working;
        self.refresh_pending_input_preview();
        self.request_redraw();
    }

    pub(crate) fn on_tiffany_orchestrator_run_finished(&mut self) {
        self.input_queue.user_turn_pending_start = false;
    ...
    pub(super) fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "tiffany-loop is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    pub(super) fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        ...
        match rate_limit_reached_type {
            Some(RateLimitReachedType::WorkspaceOwnerCreditsDepleted) => {
                self.on_error(
                    "You're out of credits. Your workspace is out of credits. Add credits to continue using tiffany-loop."
                        .to_string(),
                );
            }
