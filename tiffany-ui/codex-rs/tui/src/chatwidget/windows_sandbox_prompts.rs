...
impl ChatWidget {
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        if allow_unelevated {
            lines.push(line![
                "You can still use tiffany-loop in a non-admin sandbox. It carries greater risk if prompt injected."
            ]);
        } else {
            lines.push(line![
                "Your organization requires the default sandbox before tiffany-loop can continue."
            ]);
        }
        lines.push(line![
        ...
        if allow_unelevated {
            items.push(SelectionItem {
                name: "Use tiffany-loop with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
