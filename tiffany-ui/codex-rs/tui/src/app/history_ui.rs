...
const DESKTOP_THREAD_OPENED_MESSAGE: &str = "Opened this session in desktop app.";
...
impl App {
    ...
    pub(super) fn clear_ui_header_lines(&self, width: u16) -> Vec<Line<'static>> {
        self.clear_ui_header_lines_with_version(width, crate::version::TIFFANY_LOOP_VERSION)
    }
...
fn desktop_thread_open_error_message(err: &str) -> String {
    format!(
        "Failed to open this session in desktop app: {err}. Install or launch desktop app and try again."
    )
}
...
fn open_desktop_thread_url(url: &str) -> Result<(), String> {
    ...
    let output = std::process::Command::new("powershell.exe")
        ...
        .arg(&script)
        .output()
        .map_err(|err| format!("failed to launch desktop app through PowerShell: {err}"))?;
    ...
    if stderr.is_empty() {
        Err(format!(
            "failed to launch desktop app through PowerShell with {}",
            output.status
        ))
    } else {
        Err(stderr)
...
if (-not (Test-Path $app)) {{
    Write-Error "desktop app app bundle not found at $app"
    exit 1
}}
...
fn open_desktop_thread_url(_url: &str) -> Result<(), String> {
    Err("desktop app is only available on macOS and Windows".to_string())
}
