...
impl App {
    pub(super) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::CodexOp(op) => {
                if let Some(prompt) = crate::tiffany_orchestrator::prompt_from_app_command(&op)
                    && self.handle_tiffany_orchestrator_prompt(tui, prompt)
                {
                    return Ok(AppRunControl::Continue);
                }
                        ...
                        Ok(_) => {
                                ...
                                self.chat_widget.world_writable_warning_details()
                            {
                                ...
                                self.chat_widget.add_plain_history_lines(vec![
                                    ...
                                    Line::from(vec![
                                        "  ".into(),
                                        "tiffany-loop can now safely edit files and execute commands on your computer"
                                            .dark_gray(),
                                    ]),
                                ]);
                            } else {
    ...
    fn handle_tiffany_orchestrator_prompt(&mut self, tui: &mut tui::Tui, prompt: String) -> bool {
        if self.tiffany_orchestrator.is_none() {
            return false;
        }

        self.chat_widget
            .on_tiffany_orchestrator_submission_intercepted();
        ...
        if self.tiffany_orchestrator_running {
            self.chat_widget.queue_tiffany_orchestrator_prompt(prompt);
        } else {
            self.start_tiffany_orchestrator_run(prompt);
        }
        tui.frame_requester().schedule_frame();
        true
    }

    pub(super) fn start_tiffany_orchestrator_run(&mut self, prompt: String) {
        let Some(config) = self.tiffany_orchestrator.clone() else {
            return;
        };
        let context_turn_count = self.tiffany_orchestrator_turns.len();
        let contextual_prompt = crate::tiffany_orchestrator::contextual_prompt(
            &self.tiffany_orchestrator_turns,
            &prompt,
        );
        self.tiffany_orchestrator_running = true;
        self.chat_widget.on_tiffany_orchestrator_run_started();
        crate::tiffany_orchestrator::spawn_event_bridge(
            self.app_event_tx.clone(),
            config.launch(prompt, contextual_prompt, context_turn_count),
        );
    }
    ...
    fn handle_tiffany_orchestrator_roles_command(&mut self, tui: &mut tui::Tui, args: String) {
        let Some(config) = self.tiffany_orchestrator.clone() else {
            self.insert_history_cell(
                tui,
        ...
        crate::tiffany_orchestrator::spawn_roles_command(self.app_event_tx.clone(), config, args);
        tui.frame_requester().schedule_frame();
    }

    fn handle_tiffany_orchestrator_provider_command(&mut self, tui: &mut tui::Tui, args: String) {
        let Some(config) = self.tiffany_orchestrator.clone() else {
            self.insert_history_cell(
                tui,
                Box::new(history_cell::PlainHistoryCell::new(vec![
        ...
        crate::tiffany_orchestrator::spawn_provider_command(
            self.app_event_tx.clone(),
            config,
            args,
        );
    ...
    fn handle_tiffany_orchestrator_thread_command(&mut self, tui: &mut tui::Tui, args: String) {
        let Some(config) = self.tiffany_orchestrator.clone() else {
            self.insert_history_cell(
                tui,
                Box::new(history_cell::PlainHistoryCell::new(vec![
    ...
    fn handle_tiffany_orchestrator_continue_command(&mut self, tui: &mut tui::Tui, args: String) {
        let Some(config) = self.tiffany_orchestrator.clone() else {
            self.insert_history_cell(
                tui,
                Box::new(history_cell::PlainHistoryCell::new(vec![
        ...
        crate::tiffany_orchestrator::spawn_continue_command(
            self.app_event_tx.clone(),
            config,
            args,
        );
    ...
    fn handle_tiffany_orchestrator_run_finished(&mut self, tui: &mut tui::Tui) {
        self.tiffany_orchestrator_running = false;
        self.chat_widget.on_tiffany_orchestrator_run_finished();
        tui.frame_requester().schedule_frame();
    }
}
