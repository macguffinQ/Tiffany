...
pub(crate) fn print_auto_backup_start(startup_error: &LocalStateDbStartupError) {
    eprintln!("tiffany-loop could not start because its local database appears to be damaged.");
    eprintln!(
        "Moving the damaged local database aside so tiffany-loop can rebuild it from saved data."
    );
...
pub(crate) fn confirm_fresh_start_rebuild(
    ...
    eprintln!("tiffany-loop rebuilt its local database.");
    eprintln!(
        "tiffany-loop detected a damaged local database, moved it into a backup folder, and will continue startup with a fresh database."
    );
...
pub(crate) fn print_locked_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!(
        "tiffany-loop could not start because another tiffany-loop process is using its local data."
    );
    eprintln!("Quit any other copies of tiffany-loop that may still be running, then try again.");
    print_technical_details(startup_error);
}
