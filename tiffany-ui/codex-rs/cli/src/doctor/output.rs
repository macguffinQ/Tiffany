...
pub(super) fn render_human_report(report: &DoctorReport, options: HumanOutputOptions) -> String {
    ...
    let _ = writeln!(
        out,
        "{} {}",
        bold("tiffany-loop Doctor", options),
...
mod tests {
    ...
    fn sample_report() -> DoctorReport {
        let checks = vec![
            ...
            .remediation("Configure a tiffany-loop provider credential."),
            DoctorCheck::new(
                "updates.status",
                "updates",
                CheckStatus::Ok,
    ...
    fn render_human_report_includes_details_by_default_without_color() {
        ...
        let expected = format!(
            "\
tiffany-loop Doctor v0.0.0

Notes
   ⚠ terminal     narrow terminal
   ✗ auth         token expired - Configure a tiffany-loop provider credential.
─────────────────────────────────────────────────────────────
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Configure a tiffany-loop provider credential.
-------------------------------------------------------------
