...
use tiffany::TiffanyOrchestratorCommand;
...
enum Subcommand {
    /// run tiffany-loop non-interactively.
    #[clap(visible_alias = "e")]
    Exec(ExecCli),

    /// run tiffany-loop orchestrator inside tiffany-loop TUI.
    ...
    /// Manage external MCP servers for tiffany-loop.
    Mcp(McpCli),

    /// Manage tiffany-loop plugins.
    Plugin(PluginCli),

    /// start tiffany-loop as an MCP server (stdio).
    ...
    /// update tiffany-loop to the latest version.
    Update,

    /// Diagnose local tiffany-loop installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within the tiffany-loop sandbox.
    ...
    /// Apply the latest diff produced by tiffany-loop agent as a `git apply` to your local working tree.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of tiffany-loop.
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for tiffany-loop tooling.
    #[clap(hide = true)]
...
fn reject_removed_account_command_prompt(prompt: Option<&str>) {
    let Some(prompt) = prompt else {
        return;
    };
    let command = prompt.trim();
    if command.eq_ignore_ascii_case("login") || command.eq_ignore_ascii_case("logout") {
        eprintln!(
            "tiffany-loop account login has been removed. Configure providers with `tiffany orchestrator` -> `/provider`, or run `orchestrator config provider`."
        );
        std::process::exit(1);
    }
}
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating tiffany-loop via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart tiffany-loop.");
    Ok(())
}
...
fn main() -> anyhow::Result<()> {
    tiffany::install_tiffany_config_home()?;
...
async fn cli_main(
    ...
    match subcommand {
        None => {
            reject_removed_account_command_prompt(interactive.prompt.as_deref());
        ...
        Some(Subcommand::Orchestrator(cmd)) => {
            ...
            if cmd.legacy {
                tiffany::run_orchestrator_bridge(cmd)?;
            } else {
                let launch = cmd.native_launch()?;
                prepend_config_flags(
                    &mut interactive.config_overrides,
                    root_config_overrides.clone(),
                );
...
async fn run_exec_server_command(
    ...
    let codex_self_exe = arg0_paths
        .codex_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("tiffany-loop executable path is not configured"))?;
...
async fn load_exec_server_remote_auth_provider(
    ...
    let auth = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires API key authentication; set CODEX_API_KEY or configure a provider credential",
    )
    .await?;
...
fn unsupported_subcommand_name_for_strict_config(
    ...
    match subcommand {
        ...
        Some(Subcommand::Orchestrator(_)) => Some("orchestrator"),
...
async fn run_interactive_tui(
    ...
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". tiffany-loop interactive TUI may not work in this terminal."
        );
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged tiffany-loop local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
mod tests {
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        ...
        assert!(
            help.contains("Usage: tiffany plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: tiffany plugin marketplace add"),
            ("list", "Usage: tiffany plugin marketplace list"),
            ("upgrade", "Usage: tiffany plugin marketplace upgrade"),
            ("remove", "Usage: tiffany plugin marketplace remove"),
        ] {
