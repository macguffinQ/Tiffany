...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load tiffany-loop config")
}
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary = "package-manager update would target a different install".to_string();
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update tiffany-loop so the JS shim provides CODEX_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn auth_check(config: &Config) -> DoctorCheck {
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check = check.remediation(
                    "Configure a tiffany-loop provider credential or provide a supported auth env var.",
                );
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no tiffany-loop credentials were found",
        )
        .details(details)
        .remediation(
            "Configure a tiffany-loop provider credential or provide an API key through a supported auth env var.",
        ),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or configure a tiffany-loop provider credential."),
    }
}
...
mod tests {
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running tiffany-loop."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running tiffany-loop.".to_string())
        );
    }
