...
正式产品名和主安装命令都是 **tiffany-loop**。旧的 `tiffany` UI 命令和底层 `orchestrator` runtime 命令仍保留，用于兼容已有脚本和高级用法。
...
| `tiffany` | 兼容 UI 别名 | 仍然调用 `tiffany orchestrator` 的旧脚本 |
...
- **交接能力**：可生成 Claude/Codex/Gemini CLI handoff 包；也可用 `/continue open <role|claude|codex|gemini>` 暂停 Tiffany，打开原始 CLI 会话继续工作，返回后把 Claude/Codex/Gemini transcript 事件和 git status/stat/patch 作为 typed history 保存。
...
macOS 源码构建如果在 Xcode 更新后失败，或 `xcode-select` 指向 `/Applications/Xcode-beta.app`，先运行 `orchestrator doctor`；doctor 会显示当前 developer dir、`clang`、`xcodebuild`，并给出切回稳定 Xcode 或 Command Line Tools 的 `sudo xcode-select -s ...` 命令。
...
`orchestrator tui` 会优先查找同目录或 `PATH` 里的 `tiffany-loop` 并启动 `tiffany-loop orchestrator`。找不到 UI binary 时才使用旧兼容入口；需要强制旧入口时使用：
...
- `tiffany-loop "..."` 原生 tiffany-loop TUI 事件 adapter
